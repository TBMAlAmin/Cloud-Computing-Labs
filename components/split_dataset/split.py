...
def main():
    ...
    # First split: train (60%) vs remainder (40%)
    ...
    # Remainder is 40%, split into val/test/deploy as 15/15/10 of total
    # Relative within remainder: 15/40, 15/40, 10/40
    remainder_val_ratio = args.val_ratio / (args.val_ratio + args.test_ratio + args.deploy_ratio)

    val_df, temp_df = train_test_split(
        remainder_df,
        test_size=(1 - remainder_val_ratio),
        random_state=args.seed,
        shuffle=True,
    )

    # temp_df now represents test+deploy = 25% of total
    temp_test_ratio = args.test_ratio / (args.test_ratio + args.deploy_ratio)

    test_df, deploy_df = train_test_split(
        temp_df,
        test_size=(1 - temp_test_ratio),
        random_state=args.seed,
        shuffle=True,
    )
